...
def index():
    ...
    input_required = uv_sim and uv_sim.cpu.waiting_for_input and uv_sim.cpu.memory[uv_sim.cpu.input_operand] == 0
...
def load():
    global uv_sim
    if not uv_sim:
        flash('Please set the memory size first.')
        return redirect(url_for('index'))
...
def run():
    global uv_sim
    if uv_sim is None:
        flash('Please set the memory size and load a program first.')
        return redirect(url_for('index'))

    uv_sim.cpu.reset_state()  # Reset the CPU state before running
    uv_sim.run()
    if uv_sim.cpu.waiting_for_input:
        flash('Program running. Waiting for input...')
    else:
...
def rerun():
    ...
    uv_sim.cpu.rerun()  # Use the new rerun method
    flash('Program re-executed with current memory state.')
    return redirect(url_for('index'))
    ...
    if 0 <= memory_location < len(uv_sim.cpu.memory):
        try:
            ...
            if -999999 <= instruction <= 999999:
                # Update memory and print the new state for debugging
                uv_sim.cpu.memory[memory_location] = instruction
                print(f"Updated memory at {memory_location}: {instruction}")  # Debug log
                print(data)
